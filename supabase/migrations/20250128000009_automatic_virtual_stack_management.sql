/*
  Automatic Virtual Stack Management
  ...
  IF v_both_40ft THEN
    ...
    IF v_virtual_stack_id IS NULL THEN
      ...
      SELECT 
        ...
        CASE 
          WHEN s1.row_tier_config IS NOT NULL AND s1.row_tier_config != 'null'::jsonb THEN
            (SELECT SUM((config->>'maxTiers')::integer)
             FROM jsonb_array_elements(s1.row_tier_config) AS config)
          ELSE s1.rows * s1.max_tiers
        END,
        0, -- current_occupancy
        s1.position_x + 2.5, -- Position between the two stacks
        s1.position_y,
        s1.position_z,
        s1.width,
        s1.length,
    ...
    -- 4. Generate locations for virtual stack using SQL
    -- Note: Location generation will be handled by the application layer
    -- The trigger will notify the application to generate locations
    RAISE NOTICE '📍 Virtual stack S% created - locations will be generated by application', 
      LPAD(v_virtual_stack_number::text, 2, '0');
