/*
  Automatic Virtual Stack Management - Simple Version
...
DECLARE
  ...
  v_pairing_id UUID;
  v_both_40ft BOOLEAN;
  -- First stack properties
  v_section_id TEXT;
  v_rows INTEGER;
  v_max_tiers INTEGER;
  v_row_tier_config JSONB;
  v_capacity INTEGER;
  v_position_x DECIMAL;
  v_position_y DECIMAL;
  v_position_z DECIMAL;
  v_width DECIMAL;
  v_length DECIMAL;
BEGIN
  ...
  -- Get first stack properties for virtual stack creation
  SELECT section_id, rows, max_tiers, row_tier_config, capacity,
         position_x, position_y, position_z, width, length
  INTO v_section_id, v_rows, v_max_tiers, v_row_tier_config, v_capacity,
       v_position_x, v_position_y, v_position_z, v_width, v_length
  FROM stacks 
  WHERE yard_id = p_yard_id AND stack_number = p_first_stack_number;
  
  -- Get second stack information
  SELECT id, (container_size = '40ft' AND is_active = true)
  ...
  IF v_both_40ft THEN
    ...
    IF v_virtual_stack_id IS NULL THEN
      INSERT INTO stacks (
        ...
        CASE 
          WHEN v_row_tier_config IS NOT NULL AND v_row_tier_config != 'null'::jsonb THEN
            (SELECT SUM((config->>'maxTiers')::integer)
             FROM jsonb_array_elements(v_row_tier_config) AS config)
          ELSE v_rows * v_max_tiers
        END,
        0, -- current_occupancy
        COALESCE(v_position_x, 0) + 2.5, -- Position between the two stacks
        COALESCE(v_position_y, 0),
        COALESCE(v_position_z, 0),
        COALESCE(v_width, 2.5),
        COALESCE(v_length, 12),
        true, -- is_virtual
        '40ft', -- container_size
        true -- is_active
