...
export class ClientPoolService {
  ...
  async assignContainerToClientStack(
    ...
    try {
      ...
      this.updateClientPoolOccupancy(request.clientCode, 1);
   ...
   * Update client pool occupancy
   */
  private updateClientPoolOccupancy(clientCode: string, change: number): void {
   ...
   */
  removeStackFromClient(stackId: string, clientCode: string): boolean {
    try {
      ...
      console.log(`Stack ${stackId} removed from client ${clientCode}`);
      return true;
    } catch (error) {
   ...
   */
  updateClientPool(clientCode: string, updates: Partial<ClientPool>): ClientPool | null {
    const pool = this.clientPools.get(clientCode);
    if (!pool) return null;
    ...
    console.log(`Updated client pool for ${clientCode}`);
    return updatedPool;
  }
  ...
  bulkAssignStacksToClient(
    ...
    console.log(`Bulk assigned ${assignments.length} stacks to client ${clientCode}`);
    return assignments;
  }
   ...
   */
  releaseContainerFromPool(containerNumber: string, clientCode: string): void {
    try {
      this.updateClientPoolOccupancy(clientCode, -1);
      console.log(`Container ${containerNumber} released from client pool ${clientCode}`);
    } catch (error) {
