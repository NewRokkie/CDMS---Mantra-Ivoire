...
export const DatePicker: React.FC<DatePickerProps> = ({
  ...
  // Close when clicking outside field + portaled overlay (avoids clipped fixed inside modals)
  useEffect(() => {
    if (!isOpen) return;
    ...
    isOpen && typeof document !== 'undefined'
      ? createPortal(
          <div ref={portalRootRef} className="fixed inset-0 z-[9998]" role="presentation">
            <div
              className="absolute inset-0 bg-black bg-opacity-20 backdrop-blur-sm animate-fade-in"
              onClick={handleBackdropClick}
              aria-hidden
            />
            <div className="absolute inset-0 flex items-center justify-center p-4 pointer-events-none">
              <div className="pointer-events-auto animate-fade-scale-in origin-center">
