...
export const GateOutModal: React.FC<GateOutModalProps> = ({
  ...
  const isContainerNumberValid = (index: number): boolean => {
    ...
    const confirmNumber = confirmContainerNumbers[index];    const validation = validateContainerNumber(containerNumber);
              ...
              <div className="flex justify-between relative z-20">
                {[1, 2].map((step) => (
                  <div key={step} className="flex flex-col items-center">
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-medium transition-all duration-300 ${step <= currentStep
                        ? 'bg-blue-600 text-white border border-blue-600'
                        : 'bg-white text-gray-500 border border-gray-300'
                    }`}>
                      {step}
                    </div>
                    <span className={`mt-1.5 text-xs font-medium transition-colors duration-300 ${step <= currentStep ? 'text-blue-600' : 'text-gray-500'} `}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                          ...
                          {selectedBooking.bookingType && (
                            <div className="flex justify-between">
                              ...
                              <span className={`px-2 py-1 text-xs font-medium rounded-full ${selectedBooking.bookingType === 'IMPORT' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'}`}>
                                {selectedBooking.bookingType}
                              </span>
                            </div>
                          )}
